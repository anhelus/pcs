...
You don't have to be a weather expert to know that the amount of rainfall during a precipitation is influenced by factors like temperature, winds, humidity, geographical location, and more. Let's imagine we have a dataset that describes the weather conditions at a given moment in time (let's say, on a daily basis) along with the millimeters of rainfall during the same period. We conduct an exploratory analysis of this data, and the results are (partially) shown in Figure 1.
